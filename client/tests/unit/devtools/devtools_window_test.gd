...
func test_freeze_buttons_emit_expected_freeze_targets() -> void:
	...
	assert_eq(emitted_targets.size(), 4)
	assert_eq(emitted_targets[0], "asteroids")
	assert_eq(emitted_targets[1], "spawns")
	assert_eq(emitted_targets[2], "collisions")
	assert_eq(emitted_targets[3], "")
...
func test_set_debug_status_updates_granular_freeze_labels() -> void:
	...
	assert_true(asteroids_status_label.text.contains("ON"))
	assert_true(spawns_status_label.text.contains("ON"))
	assert_true(collisions_status_label.text.contains("OFF"))
	assert_true(world_status_label.text.contains("OFF"))
