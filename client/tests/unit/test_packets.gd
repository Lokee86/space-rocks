...
func test_packet_builders_set_expected_type() -> void:
	var cases := [
		...
		[Packets.create_room_request_packet("", "", "ffa", "", 0, 0, "arcade_survival", 0, false, 0, 0), Packets.TYPE_CREATE_ROOM_REQUEST],
...
func test_initial_room_packet_builders_carry_trace_id() -> void:
	...
	var create_packet := Packets.create_room_request_packet(trace_id, "profile-1", "custom", "owner_assigned", 0, 8, "score_attack", 4, false, 2500, 0)
	...
	assert_eq(create_packet[Packets.FIELD_TARGET_KILLS], 0)

	var join_packet := Packets.join_room_request_packet("TEST", trace_id)
	assert_eq(join_packet[Packets.FIELD_TRACE_ID], trace_id)

	var single_player_packet := Packets.start_single_player_request_packet("profile-1", trace_id, 4, "auto_balanced", "", 2, "team_deathmatch", 0, true, 0, 10)
	assert_eq(single_player_packet[Packets.FIELD_TRACE_ID], trace_id)
	assert_eq(single_player_packet[Packets.FIELD_MAX_PLAYERS], 4)
	assert_eq(single_player_packet[Packets.FIELD_TEAM_STRUCTURE], "auto_balanced")
	assert_eq(single_player_packet[Packets.FIELD_TEAM_ASSIGNMENT_MODE], "")
	assert_eq(single_player_packet[Packets.FIELD_TEAM_COUNT], 2)
	assert_eq(single_player_packet[Packets.FIELD_PRESET_ID], "team_deathmatch")
	assert_eq(single_player_packet[Packets.FIELD_STARTING_LIVES], 0)
	assert_eq(single_player_packet[Packets.FIELD_INFINITE_LIVES], true)
	assert_eq(single_player_packet[Packets.FIELD_TARGET_SCORE], 0)
	assert_eq(single_player_packet[Packets.FIELD_TARGET_KILLS], 10)

	var empty_trace_packet := Packets.create_room_request_packet("", "", "ffa", "", 0, 0, "arcade_survival", 0, false, 0, 0)
