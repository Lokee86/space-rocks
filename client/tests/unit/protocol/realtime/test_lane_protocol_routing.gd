...
func test_lane_packet_families_route_to_realtime_router() -> void:
	var router := RealtimeRouter.new()

	router.route_packet_for_protocol_mode({"type": LaneMetadata.PACKET_FAMILY_WORLD[0], "baseline_id": "b1", "sequence": 1, "snapshot_id": "s1", "is_final_chunk": true}, "lane_protocol")
	assert_true(router.baseline_tracker.is_lane_synced(LaneMetadata.LANE_WORLD))

	router.route_packet_for_protocol_mode({"type": LaneMetadata.PACKET_FAMILY_WORLD[1], "baseline_id": "b1", "sequence": 2}, "lane_protocol")
	router.route_packet_for_protocol_mode({"type": LaneMetadata.PACKET_FAMILY_OVERLAY[0], "baseline_id": "b2", "sequence": 1, "snapshot_id": "o1", "is_final_chunk": true}, "lane_protocol")
	assert_true(router.baseline_tracker.is_lane_synced(LaneMetadata.LANE_OVERLAY))

	router.route_packet_for_protocol_mode({"type": LaneMetadata.PACKET_FAMILY_OVERLAY[1], "baseline_id": "b2", "sequence": 2}, "lane_protocol")
	router.route_packet_for_protocol_mode({"type": LaneMetadata.PACKET_FAMILY_SESSION[0], "baseline_id": "b3", "sequence": 1, "snapshot_id": "u1", "is_final_chunk": true}, "lane_protocol")
	assert_true(router.baseline_tracker.is_lane_synced(LaneMetadata.LANE_SESSION))

	router.route_packet_for_protocol_mode({"type": LaneMetadata.PACKET_FAMILY_SESSION[1], "baseline_id": "b3", "sequence": 2}, "lane_protocol")
	router.route_packet_for_protocol_mode({"type": LaneMetadata.PACKET_FAMILY_EVENT[0], "batch_id": "batch-1", "events": [{"event_id": "event-1", "type": "spark", "payload": {}}]}, "lane_protocol")
...
func test_lowercase_lane_fixtures_route_through_realtime_router() -> void:
	var router := RealtimeRouter.new()
	...
	router.route_packet_for_protocol_mode(world_packet, "lane_protocol")
	assert_true(router.baseline_tracker.is_lane_synced(LaneMetadata.LANE_WORLD))
	...
	router.route_packet_for_protocol_mode(overlay_packet, "lane_protocol")
	assert_true(router.baseline_tracker.is_lane_synced(LaneMetadata.LANE_OVERLAY))
	...
	router.route_packet_for_protocol_mode(session_packet, "lane_protocol")
	assert_true(router.baseline_tracker.is_lane_synced(LaneMetadata.LANE_SESSION))
