...
var world_sync: WorldSync = null
var respawn_flow: GameplayRespawnFlow = null
...
func configure(
	world_sync_ref: WorldSync,
	respawn_flow_ref: GameplayRespawnFlow,
...
func apply_state(state: Dictionary) -> void:
	...
	if status != PlayerLifecycle.STATUS_ACTIVE or respawn_flow == null:
		return
	var stale: bool = match_end_flow != null and match_end_flow.has_stale_dead_presentation()
...
func apply_lane_state(world_lane_state: WorldLaneState, session_lane_state: SessionLaneState, self_id: String) -> void:
	...
	var stale: bool = hud_flow.has_dead_presentation()
	if !respawn_flow.should_restore_alive_hud(world_ships, player_lifecycle, self_id, player, stale):
		return
	if world_sync != null:
		world_sync.clear_view_target_player()
...
func _apply_pending_respawn(session_lane_state: SessionLaneState, self_id: String) -> void:
	...
	if player_session is Dictionary and player_session.has("lives"):
	...
	if entering_pending and player != null:
		player.stop_transient_effects()
	var has_dead_presentation: bool = hud_flow.has_dead_presentation()
