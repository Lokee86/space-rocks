...
const WEAPON_POINT_SECONDARY_1 := "secondary_1"
...
func _ready() -> void:
	...
	_populate_optional_selector(secondary_weapon_option, [], "owned_weapon_id", "")
	for slot in MODULE_SLOTS:
		_populate_optional_selector(module_options[slot], [], "owned_module_id", "")
...
func configure(build_options: Dictionary, loadout_selection: Dictionary) -> void:
	...
	_populate_optional_selector(
		secondary_weapon_option,
		_weapon_options_for_point(_array_value(options, "eligible_weapons"), WEAPON_POINT_SECONDARY_1),
		"owned_weapon_id",
		str(selected_weapons.get(WEAPON_POINT_SECONDARY_1, ""))
	)
	for slot in MODULE_SLOTS:
		_populate_optional_selector(
			module_options[slot],
...
func _on_apply_pressed() -> void:
	...
	var selected_primary_id := _selected_id(primary_weapon_option)
	if selected_ship_id == "" or selected_primary_id == "":
		status_label.text = "ERROR: SELECT A SHIP AND PRIMARY_1 WEAPON"
		return

	var selected_weapons := {WEAPON_POINT_PRIMARY_1: selected_primary_id}
	var selected_secondary_id := _selected_id(secondary_weapon_option)
	if selected_secondary_id != "":
		selected_weapons[WEAPON_POINT_SECONDARY_1] = selected_secondary_id
	...
	var selected_starting_ammo := {}
	for point in selected_weapons:
		if _starting_ammo_by_point.has(point):
			selected_starting_ammo[point] = _starting_ammo_by_point[point]
	...
	submit_requested.emit({
		...
		"starting_ammo_by_point": selected_starting_ammo,
	})
...
func _populate_selector(selector: OptionButton, options: Array, value_key: String, selected_id: String) -> void:
	selector.clear()
	_add_options(selector, options, value_key)
	if selector.item_count == 0:
		_set_selector_empty(selector, "NO OPTIONS AVAILABLE")
		return
	selector.disabled = false
	_select_metadata(selector, selected_id)


func _populate_optional_selector(selector: OptionButton, options: Array, value_key: String, selected_id: String) -> void:
	selector.clear()
...
func _add_options(selector: OptionButton, options: Array, value_key: String) -> void:
	for option in options:
		...
		if option_id == "":
			continue
		selector.add_item(_option_label(option, option_id))
		selector.set_item_metadata(selector.item_count - 1, option_id)


func _select_metadata(selector: OptionButton, selected_id: String) -> void:
