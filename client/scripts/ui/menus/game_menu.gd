...
var primary_action := Constants.GAME_MENU_PRIMARY_ACTION_RESUME
	...
	if normalized_session_mode == Constants.SESSION_MODE_MULTIPLAYER:
		if game_over:
			if has_spectate_targets:
				...
				primary_action = Constants.GAME_MENU_PRIMARY_ACTION_WAITING
				set_primary_text("Waiting")
				set_primary_enabled(false)
		else:
			primary_action = Constants.GAME_MENU_PRIMARY_ACTION_RESUME
...
func _on_primary_action_pressed() -> void:
	...
	if primary_action == Constants.GAME_MENU_PRIMARY_ACTION_LOBBY:
		lobby_requested.emit()
		return
	if primary_action == Constants.GAME_MENU_PRIMARY_ACTION_SPECTATE:
...
func _primary_label_name(text: String) -> String:
	if _normalized_state(text) == Constants.GAME_MENU_PRIMARY_ACTION_LOBBY:
		return "Lobby"
	if _normalized_state(text) == Constants.GAME_MENU_PRIMARY_ACTION_SPECTATE:
		return "Spectate"
	if _normalized_state(text) == Constants.GAME_MENU_PRIMARY_ACTION_WAITING:
		return "Waiting"
