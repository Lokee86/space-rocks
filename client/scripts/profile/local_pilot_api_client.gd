...
func list_profiles(token := ""):
	return await api_http_client.get_json(ApiConfigScript.player_data_local_profiles_path(), token)


func get_default_profile(token := ""):
	return await api_http_client.get_json(ApiConfigScript.player_data_local_profiles_default_path(), token)
...
func create_profile(display_name: String, seed_from_guest_stats: bool, token := ""):
	...
	return await api_http_client.post_json(ApiConfigScript.player_data_local_profiles_path(), body, token)


func delete_profile(local_profile_id: String, token := ""):
	return await api_http_client.delete_json(ApiConfigScript.player_data_local_profile_path(local_profile_id), {}, token)
...
func update_profile_display_name(local_profile_id: String, display_name: String, token := ""):
	...
	return await api_http_client.put_json(ApiConfigScript.player_data_local_profile_path(local_profile_id), body, token)
...
func set_default_profile(identity_kind: String, local_profile_id := "", token := ""):
	...
	return await api_http_client.put_json(ApiConfigScript.player_data_local_profiles_default_path(), body, token)
