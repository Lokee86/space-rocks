...
var measurement_toggle_route: Callable


func configure(state_context_ref, overlay_context_ref, hotkey_flow_ref, window_toggle_route_ref: Callable, measurement_toggle_route_ref: Callable = Callable()) -> void:
	...
	measurement_toggle_route = measurement_toggle_route_ref
...
func process(required_lane_baselines_synced: bool) -> void:
	...
	if Input.is_action_just_pressed("DevToggle9"):
		if Input.is_key_pressed(KEY_SHIFT):
			if measurement_toggle_route.is_valid():
				measurement_toggle_route.call()
		elif overlay_context != null:
