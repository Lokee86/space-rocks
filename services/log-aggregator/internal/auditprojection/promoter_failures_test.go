...
func TestPromoterRequirements(t *testing.T) {
	...
	if _, err := p.Promote(context.Background(), sourceRecord(true)); !errors.Is(err, ErrStoreRequired) {
		t.Fatalf("store: %v", err)
	}
	p.Store = &fakeStore{}
	p.Sanitize = nil
	if _, err := p.Promote(context.Background(), sourceRecord(true)); !errors.Is(err, ErrSanitizerRequired) {
...
func TestPromoterOriginalPayloadValidation(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			r := sourceRecord(true)
			r.Payload = json.RawMessage(tc.payload)
...
func TestPromoterSanitizedOutputValidation(t *testing.T) {
	...
	for _, tc := range outputs {
		t.Run(tc.name, func(t *testing.T) {
			...
			if !errors.Is(err, ErrInvalidSanitizedOutput) {
				t.Fatalf("got %v", err)
			}
		})
	}
	...
	if _, err := p.Promote(context.Background(), sourceRecord(true)); !errors.Is(err, want) {
...
func TestPromoterPolicyAndGenerationFailures(t *testing.T) {
	...
	if result, err := p.Promote(context.Background(), sourceRecord(true)); !errors.Is(err, policyErr) || result.Promoted || calls != 0 || store.saves != 0 {
	...
	if result, err := p.Promote(context.Background(), sourceRecord(true)); !errors.Is(err, generatorErr) || result.Promoted {
		t.Fatalf("uuid error: %#v %v", result, err)
	}

	p = configuredPromoter(&fakeStore{})
	p.NewUUID = func() (string, error) { return "bad", nil }
	if result, err := p.Promote(context.Background(), sourceRecord(true)); !errors.Is(err, ErrInvalidGeneratedAuditID) || result.Promoted {
		t.Fatalf("uuid validation: %#v %v", result, err)
	}

	store = &fakeStore{err: errors.New("save")}
	p = configuredPromoter(store)
	if result, err := p.Promote(context.Background(), sourceRecord(true)); !errors.Is(err, store.err) || result.Promoted || store.saves != 1 {
		t.Fatalf("store error: %#v %v", result, err)
	}
}
