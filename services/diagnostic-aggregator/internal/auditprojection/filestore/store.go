...
func (s *Store) Save(ctx context.Context, record audit.Record) error {
	...
	if !validUUID(record.AuditEventID) {
		return audit.ErrInvalidAuditEventID
	}
...
func (s *Store) Get(ctx context.Context, id string) (audit.Record, error) {
	...
	if s == nil || s.root == "" {
		return audit.Record{}, ErrInvalidRoot
	}
	if !validUUID(id) {
		return audit.Record{}, audit.ErrInvalidAuditEventID
	}
	file, err := os.Open(filepath.Join(s.root, id+".json"))
	if errors.Is(err, os.ErrNotExist) {
		return audit.Record{}, &audit.RecordNotFoundError{AuditEventID: id}
	}
	if err != nil {
		return audit.Record{}, err
	}
	...
	if int64(len(data)) > s.maxBytes {
		return audit.Record{}, ErrRecordTooLarge
	}
	var record audit.Record
	decoder := json.NewDecoder(bytes.NewReader(data))
	if err := decoder.Decode(&record); err != nil {
		return audit.Record{}, fmt.Errorf("%w: %v", ErrInvalidStoredData, err)
	}
	var extra any
	if err := decoder.Decode(&extra); err != io.EOF {
		return audit.Record{}, ErrInvalidStoredData
	}
	if record.AuditEventID != id {
		return audit.Record{}, ErrInvalidStoredData
	}
	return record, nil
}
