...
func TestHandleGameplayPacketQueuesResyncRequest(t *testing.T) {
	session := &resyncGameplaySessionFake{
		room:     rooms.NewRoom("room-1", rooms.RoomStateInGame, game.New()),
		playerID: "player-1",
	}
	packet := game.ClientPacket{Type: game.PacketTypeResyncRequest, Lane: string(realtime.LaneWorld), BaselineID: "baseline-4", Sequence: 4, Reason: "wrong_baseline"}
	...
	if got := session.requests[0]; got.Lane != realtime.LaneWorld || got.BaselineID != "baseline-4" || got.Sequence != 4 || got.Reason != "wrong_baseline" {
...
func TestHandleGameplayPacketRejectsInvalidResyncContext(t *testing.T) {
	tests := []struct {
		...
		{"invalid lane", rooms.NewRoom("room-1", rooms.RoomStateInGame, game.New()), "player-1"},
		{"nil room", nil, "player-1"},
		{"empty player", rooms.NewRoom("room-1", rooms.RoomStateInGame, game.New()), ""},
		{"nil game", rooms.NewRoom("room-1", rooms.RoomStateInGame, nil), "player-1"},
	}
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			session := &resyncGameplaySessionFake{room: tc.room, playerID: tc.playerID}
			lane := string(realtime.LaneWorld)
			if tc.name == "invalid lane" {
				lane = "unsupported"
			}
			if HandleGameplayPacket(session, game.ClientPacket{Type: game.PacketTypeResyncRequest, Lane: lane}) {
