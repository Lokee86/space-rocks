...
func TestSplitWorldHotUpdatesDeletedRouteIsRemoved(t *testing.T) {
	...
	result := SplitWorldHotUpdates(worldDelta, cohort, DefaultHotLaneOffloadPolicy())
	if _, exists := result.CohortState.AsteroidRoutes["asteroid-1"]; exists {
		t.Fatalf("expected missing asteroid route to be removed")
	}
}
...
func TestSplitWorldHotUpdatesNewEntityAssignmentIsDeterministic(t *testing.T) {
	...
	if got := result.CohortState.AsteroidRoutes["asteroid-9"]; got != HotUpdateRouteAsteroids {
		t.Fatalf("asteroid-9 route = %q, want asteroids", got)
	}
	if len(result.WorldDelta.Asteroids.Updates) != 0 {
		t.Fatalf("expected asteroid updates removed from world delta, got %d", len(result.WorldDelta.Asteroids.Updates))
	}
	if result.AsteroidDelta == nil || len(result.AsteroidDelta.AsteroidUpdates) != 2 {
		t.Fatalf("expected asteroid delta with 2 updates, got %#v", result.AsteroidDelta)
	}
...
func TestSplitWorldHotUpdatesDoesNotReshuffleRoutesOnInputOrderChange(t *testing.T) {
	...
	if got := result.CohortState.AsteroidRoutes["asteroid-2"]; got != HotUpdateRouteAsteroids {
		t.Fatalf("asteroid-2 route = %q, want asteroids", got)
	}
	if got := result.CohortState.BulletRoutes["bullet-1"]; got != HotUpdateRouteBullets {
		t.Fatalf("bullet-1 route = %q, want bullets", got)
	}
	if got := result.CohortState.BulletRoutes["bullet-2"]; got != HotUpdateRouteBullets {
		t.Fatalf("bullet-2 route = %q, want bullets", got)
	}
	if len(result.WorldDelta.Asteroids.Updates) != 0 {
		t.Fatalf("expected asteroid updates removed from world delta, got %d", len(result.WorldDelta.Asteroids.Updates))
	}
	if len(result.WorldDelta.Bullets.Updates) != 0 {
		t.Fatalf("expected bullet updates removed from world delta, got %d", len(result.WorldDelta.Bullets.Updates))
	}
}

func TestSplitWorldHotUpdatesAsteroidModeStartsAt60HzBeforePacketSizing(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{name: "single update", count: 1, want: HotLaneModeFullOwned60Hz},
		{name: "large cohort", count: 300, want: HotLaneModeFullOwned60Hz},
	} {
		t.Run(tc.name, func(t *testing.T) {
			...
			if result.AsteroidMode != tc.want {
				t.Fatalf("AsteroidMode = %q, want %q", result.AsteroidMode, tc.want)
			}
			if got := len(result.WorldDelta.Asteroids.Updates); got != 0 {
				t.Fatalf("world asteroid updates = %d, want 0", got)
			}
...
func TestSplitWorldHotUpdatesBulletModeStartsAt60HzBeforePacketSizing(t *testing.T) {
	policy := DefaultHotLaneOffloadPolicy()
	for _, tc := range []struct {
		name  string
		count int
		...
		{name: "single update", count: 1, want: HotLaneModeFullOwned60Hz},
		{name: "large cohort", count: 600, want: HotLaneModeFullOwned60Hz},
	} {
		t.Run(tc.name, func(t *testing.T) {
			...
			result := SplitWorldHotUpdates(worldDelta, NewHotLaneCohortState(), policy)
			if result.BulletMode != tc.want {
				t.Fatalf("BulletMode = %q, want %q", result.BulletMode, tc.want)
			}
			if got := len(result.WorldDelta.Bullets.Updates); got != 0 {
				t.Fatalf("world bullet updates = %d, want 0", got)
			}
...
func TestSplitWorldHotUpdatesOwnsAsteroidsAndBulletsIndependently(t *testing.T) {
	...
	if result.AsteroidDelta == nil || len(result.AsteroidDelta.AsteroidUpdates) != 100 {
		t.Fatalf("expected asteroid delta with 100 updates, got %#v", result.AsteroidDelta)
	}
	if result.BulletDelta == nil || len(result.BulletDelta.BulletUpdates) != 93 {
		t.Fatalf("expected bullet delta with 93 updates, got %#v", result.BulletDelta)
	}
	if result.AsteroidMode != HotLaneModeFullOwned60Hz {
		t.Fatalf("AsteroidMode = %q, want full-owned 60hz", result.AsteroidMode)
	}
	if result.BulletMode != HotLaneModeFullOwned60Hz {
		t.Fatalf("BulletMode = %q, want full-owned 60hz", result.BulletMode)
	}
}
...
func TestSplitWorldHotUpdatesMovesStressMovementUpdatesOutOfWorld(t *testing.T) {
	...
	if result.AsteroidDelta == nil {
		t.Fatalf("expected asteroid delta to be non-nil")
	}
	if got := len(result.AsteroidDelta.AsteroidUpdates); got != 80 {
		t.Fatalf("asteroid delta updates = %d, want 80", got)
	}
	if result.BulletDelta == nil {
		t.Fatalf("expected bullet delta to be non-nil")
	}
	if got := len(result.BulletDelta.BulletUpdates); got != 80 {
		t.Fatalf("bullet delta updates = %d, want 80", got)
	}
	if result.AsteroidOffloaded != 80 {
		t.Fatalf("asteroid offloaded = %d, want 80", result.AsteroidOffloaded)
	}
	if result.BulletOffloaded != 80 {
		t.Fatalf("bullet offloaded = %d, want 80", result.BulletOffloaded)
	}
}
