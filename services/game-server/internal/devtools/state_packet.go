...
type statePacketWithDebugStatus struct {
	...
	DebugCollisionBodies []game.DevtoolsCollisionBody    `json:"debug_collision_bodies"`
	TotalAsteroids      int                             `json:"total_asteroids"`
	Events              []game.EventState               `json:"events"`
}

func WrapStatePacket(state game.StatePacket, status DebugStatus, statuses map[string]DebugStatus, collisionBodies []game.DevtoolsCollisionBody) any {
	return statePacketWithDebugStatus{
		...
		DebugCollisionBodies: collisionBodies,
