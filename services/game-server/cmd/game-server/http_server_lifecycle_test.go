...
func TestServeHTTPServerServesRequestsAndGracefullyStops(t *testing.T) {
	...
	go func() { result <- serveHTTPServer(ctx, server, listener, time.Second) }()
...
func TestServeHTTPServerReturnsServeErrors(t *testing.T) {
	...
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	err = serveHTTPServer(ctx, server, listener, time.Second)
...
func TestServeHTTPServerShutdownBound(t *testing.T) {
	...
	ctx, cancel := context.WithCancel(context.Background())
	result := make(chan error, 1)
	go func() { result <- serveHTTPServer(ctx, server, listener, 20*time.Millisecond) }()
