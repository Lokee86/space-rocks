"""Strict TypeScript constants parser for pull."""
...
LINE_RE = re.compile(r"^export const (?P<name>[A-Z][A-Z0-9]*(?:_[A-Z0-9]+)*) = (?P<value>.+);$")
...
def parse_constants(text: str) -> tuple[tuple[str, Any], ...]:
    ...
    for line_number, line in _canonical_lines(text):
        ...
        if match is None:
            raise ConstantsParseError(f"line {line_number}: expected canonical TypeScript const assignment")
