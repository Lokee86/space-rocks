...
def test_pull_constants_fails_when_constants_sot_uses_multiple_paths(
    tmp_path: Path,
    capsys: pytest.CaptureFixture[str],
) -> None:
    config_path = write_project(tmp_path)
    (tmp_path / "shared/game_data.base.toml").write_text(
        """
[constants.gameplay]
player_speed = 420.0
""".strip()
    ...
    (tmp_path / "shared/game_data.extra.toml").write_text(
        """
[constants.client]
client_scale = 2
""".strip()
    ...
    config_path.write_text(
        """
[sot.constants]
paths = ["shared/game_data.base.toml", "shared/game_data.extra.toml"]
...
""".strip()
    ...
    captured = capsys.readouterr()
    assert exit_code == 2
    assert "pull error: constants pull supports only one SoT path" in captured.err
