...
| HTTP API contracts              | `shared/contracts/http/openapi.yaml`                                                   | Rails controllers and tests implement the contract; no generated controllers or clients currently exist                                                                                                                                                                                 | Rails API server, client HTTP API wrappers, game-server auth and match-result integration, player-data HTTP handlers  | Rails OpenAPI contract tests and controller tests                                            | realtime packets, data-sync domains, Rails migrations, runtime OpenAPI middleware                 |
...
OpenAPI does not own Rails migrations, Rails strong params, generated clients, realtime packet contracts, or player-data runtime packets.
