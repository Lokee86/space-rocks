...
- Timed asteroid behavior is view-driven, not global ambient spawning. `client_config` carries the visible world dimensions, the server stores that config on the player session and camera view, timed spawns are placed just outside the active camera/player view, their velocity generally points toward the camera/player target with configured randomness, and despawn uses the camera/view bounds plus despawn margin. After the networking routing split, `client_config` was not forwarded to `Game.HandlePacket`, so the camera view fell back to full world dimensions and asteroids appeared to spawn/despawn across the whole playfield.
