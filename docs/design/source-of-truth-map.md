...
| HTTP API contracts | `shared/contracts/http/openapi.yaml` | Rails controllers and controller tests in `services/api-server/`; game-server-hosted data-handler profile reads during the in-process runtime phase | `bundle exec rails test test/contracts/openapi_contract_test.rb` | Active | Rails migration layout, runtime middleware, data-sync domains |
...
- `shared/contracts/http/openapi.yaml` owns HTTP request and response contracts, including the game-server-hosted data-handler profile endpoint used during the in-process runtime phase.
