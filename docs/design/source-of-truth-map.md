...
- The embedded SQLite local-profile store exists only in `embedded_sqlite` builds; no-tag/deployment builds omit that package and its `modernc.org/sqlite` dependency.
