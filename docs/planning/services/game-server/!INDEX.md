# Game Server

This index summarizes the game server docs.

Parent index: [Services](../!INDEX.md)

## Ownership

This folder owns planning docs for the authoritative game-server surface, including networking, rooms, simulation, and process concerns.

## Does Not Belong

Client presentation, API-server planning, and unrelated product design belong in their own service docs.

## Direct Files
<!-- doc-ledger:files:start -->
<!-- doc-ledger:files:end -->

## Stub Files
<!-- doc-ledger:stubs:start -->
<!-- doc-ledger:stubs:end -->

## Direct Folders
<!-- doc-ledger:folders:start -->

- [integrations](integrations/!INDEX.md) - Integrations documentation.
- [networking](networking/!INDEX.md) - Networking documentation.

- [process](process/!INDEX.md) - Process documentation.
- [rooms](rooms/!INDEX.md) - Rooms documentation.
- [simulation](simulation/!INDEX.md) - Simulation documentation.
<!-- doc-ledger:folders:end -->

## Related Docs

- [Services](../!INDEX.md)
- [Development Roadmap](../../development-roadmap.md)
- [Domain Backlog](../../domain-backlog.md)

## Notes

Use this index for game-server planning only; implementation details stay in the service code and service docs.