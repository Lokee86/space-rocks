...
- Non-hot world and ship/bullet lifecycle changes can force related hot movement emission and advance the shared world projection chain. Chunked asteroid movement and its related lifecycle/projection transition remain aligned to the permitted asteroid cadence tick rather than bypassing cadence; this is implemented protocol/realtime policy, not future transport work.
...
Physical DataChannel separation does not currently provide full send-orchestration isolation. The per-session server write pass builds and encodes the selected lanes together, preflights all selected payloads against their lane buffers, and skips the entire gameplay send tick if any selected lane cannot fit under the configured buffered-amount threshold. Isolating congested hot lanes from unrelated reliable/lifecycle progress remains transport-orchestration hardening work.
