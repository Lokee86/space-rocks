---
author: brian
created: "2026-07-19"
document_id: 019f7d55-fb2c-7401-b6e8-36cb4ff62111
document_type: general
policy_exempt: false
summary: docs/planning contains planning docs for future, unresolved, proposed, or not-yet-current work.
---
# Planning

Parent index: [Documentation](../!INDEX.md)

`docs/planning` contains planning docs for future, unresolved, proposed, or not-yet-current work.

## Ownership

This folder owns the planning index for work that has not yet become current documentation.

## Does Not Belong

- Current canonical docs.
- Legacy docs as current authority.
- Stub content as canonical planning authority.

## Direct Files
<!-- doc-ledger:files:start -->

- [development-roadmap.md](development-roadmap.md) - High-level roadmap planning.
- [domain-backlog.md](domain-backlog.md) - Cross-domain planning triage and routing.
- [p4-team-contracts-worktree-status.md](p4-team-contracts-worktree-status.md) - Isolated P4 implementation status, integration sequence, and next steps.
<!-- doc-ledger:files:end -->
## Direct Folders
<!-- doc-ledger:folders:start -->

- [data](data/!INDEX.md) - Data documentation.
- [devtools](devtools/!INDEX.md) - Devtools documentation.
- [domains](domains/!INDEX.md) - Domains documentation.
- [protocol](protocol/!INDEX.md) - Protocol documentation.
- [services](services/!INDEX.md) - Services documentation.
- [systems-design](systems-design/!INDEX.md) - Systems Design documentation.
- [web](web/!INDEX.md) - Web documentation.
<!-- doc-ledger:folders:end -->

## Stub Files

<!-- doc-ledger:stubs:start -->
<!-- doc-ledger:stubs:end -->

## Related Docs

- [Top-level docs index](../!INDEX.md)

## Notes

Planning mirrors the current documentation structure where useful, but it remains the home for future, unresolved, proposed, or not-yet-current work.
