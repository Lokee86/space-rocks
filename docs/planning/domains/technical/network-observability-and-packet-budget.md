...
- Hot movement cadence is enforced independently for ships, asteroids, and bullets using the per-session 60 Hz cadence tick. Every hot lane uses one chunk at 60 Hz, two chunks at 30 Hz, three chunks at 20 Hz, and four or more chunks at the 15 Hz floor. All chunks for an eligible logical sequence are emitted in one same-tick unordered burst; cadence does not drop below 15 Hz.
- Movement or lifecycle changes on one lane do not force unrelated hot emission. Reliable world/lifecycle candidates remain eligible while hot lanes are cadence-suppressed. Each hot lane owns an independent movement projection and transport commit, while world plus reliable entity lifecycle form one projection group.
