...
receiver_scale_8c_0b_v1: eight real clients and zero bots
```
...
The `multi_room_3x1c_7b_v1` scenario runs three independent rooms concurrently in one server process. Each room contains one real headless coordinator and seven bots, seeds 24 asteroid spawns, runs 20 continuous projectile streams, reaches game over, and validates a distinct eight-participant result. Two local development-host runs produced unique room codes and match IDs with zero skipped receiver sends and zero client send failures. They prove that the concurrent room lifecycle, result, measurement, and networking paths function, but they do not provide clean server-performance evidence: the WSL server, three Godot clients, development tools, and unrelated host activity shared one laptop. Their timing maxima must therefore be treated as host-contended observations rather than server warnings or capacity limits.

The `multi_room_matrix_v1` manifest now defines matching one, two, three, and four-room points. Every room uses one real headless coordinator, seven bots, 24 seeded asteroid spawns, 20 continuous projectile streams, and identical phase durations and seed policy. The matrix runner executes the points sequentially against separate server processes and records an explicit controlled-host declaration and note in every run. Its aggregator validates the shared workload, room counts, unique room and match identities, lifecycle success, and combined process/receiver metrics. A matrix is performance-eligible only when all four points pass and all four runs were explicitly declared controlled. The existing local three-room artifacts remain functional-only and are excluded from clean comparison.
...
3. Use the repaired churn soak as the memory-retention baseline and run the implemented one/two/three/four-room matrix on a controlled host.
