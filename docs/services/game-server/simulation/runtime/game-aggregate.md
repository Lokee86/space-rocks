...
-> immutable published presentation frame
...
* The immutable shared presentation frame in `presentationFrame`, rebuilt and published at aggregate mutation boundaries.
* A bounded derived-presentation cache retaining the current and immediately previous frame generations for immutable cross-receiver projections.
...
= entities, spatialEntries, spatialRefs, playerSessions, cameraViews, pendingPresentationEvents, presentationFrame, presentationDerived
...
Outbound realtime state reaches clients through lane-native realtime projection. protocol/realtime reads game presentation state and builds lane-native realtime packets, packetcodec handles encoding, and outbound networking delivers selected active gameplay packets over lane-specific WebRTC DataChannels using the current mixed channel policy after signaling succeeds.
...
`Game.Step` publishes one immutable shared presentation frame after each completed simulation step while `Game.mu` is held, including the match-over return path. `New`, `AddPlayer`, `RemovePlayer`, and successful presentation-visible `Control` mutations publish coherent frames immediately. `GameplayPresentationSnapshot(playerID)` briefly locks only to capture the current frame and copy that receiver's pending-event slice, then returns a receiver-scoped wrapper over the frame's shared immutable maps. Old frames are never mutated after publication and remain safe for concurrent readers.

`protocol/realtime` may attach immutable derived data to a frame generation through `PresentationDerived`. The cache retains at most two adjacent generations so overlapping receiver write loops reuse the same quantized full-world projection without retaining an unbounded frame history. Derived values are game-local and disappear with the `Game` instance.

Pending events, network-interest membership, realtime session state, baselines, sequences, delta history, cadence, hot-route state, lifecycle selection, and encoded packets remain receiver-specific and are not shared by the presentation frame or derived cache.

`GameplayPresentationSnapshot` exposes the frame-publication timestamp as `server_sent_msec`, shared by snapshots from that frame. It is not generated by the networking writer. `protocol/realtime` carries that timestamp into lane metadata; the client later combines it with the telemetry ping/pong clock-offset estimate to map server time into client monotonic time. The aggregate does not calculate client packet age.

The current benchmark boundary covers 1 player/100 asteroids/100 bullets, 8 players/100 asteroids/500 bullets, and a 16-player/500-asteroid/2,000-bullet stress case. The receiver wrapper is roughly 34 ns/op with zero allocations across the existing scenarios. Frame publication remains roughly 11.6 us for the small case, 45.7 us for the medium case, and 225.7 us for the stress case, and occurs once per publication rather than once per receiver. Continuous snapshot readers retain the Step-with-readers contention coverage. This is current measurement evidence, not an optimization prescription; see [Runtime Performance And Scale Budget](../../../../planning/domains/technical/runtime-performance-and-scale-budget.md) for the baseline and decision boundary.
...
* Published presentation frames are immutable after replacement and may be read concurrently by snapshot callers.
* `GameplayPresentationSnapshot` shares only immutable frame maps; receiver-specific event queues and realtime delivery state remain isolated.
* Derived presentation values are immutable, game-local, keyed by frame generation, and bounded to two adjacent generations.
* `server_sent_msec` is assigned at frame publication and is shared by snapshots from that frame.
...
services/game-server/internal/game/presentation_derived.go
services/game-server/internal/game/presentation_derived_test.go
...
* immutable presentation-frame publication, reuse, lifecycle publication, and concurrent snapshot reads
* simulation stepping
