...
The networking layer owns successful WebRTC delivery for active realtime gameplay packets, successful WebSocket delivery for queued one-off packets, and the post-write session state changes that follow from those successful writes. `server_sent_msec` is not generated by the writer: it is captured as the server Unix-millisecond wall-clock time when `GameplayPresentationSnapshot` is created, then carried by realtime lane projection into the encoded lane metadata.
