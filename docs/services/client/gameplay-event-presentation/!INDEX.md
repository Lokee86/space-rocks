---
author: brian
created: "2026-07-19"
document_id: 019f7d55-fb2c-7ccd-ad42-ad259d1759b2
document_type: general
policy_exempt: false
summary: This folder owns client gameplay-event-presentation documentation.
---
# Gameplay Event Presentation

Parent index: [Client](../!INDEX.md)

## Purpose

This folder owns client gameplay-event-presentation documentation.

## Ownership

This folder owns client gameplay event presentation, visual effects, local event handoff, and gameplay audio docs.

## Does Not Belong

- Server simulation authority.
- Gameplay runtime ownership.
- World sync or entity interpolation ownership.
- HUD layout ownership.
- Future planning.

## Direct Files
<!-- doc-ledger:files:start -->

- [gameplay-audio-flow.md](gameplay-audio-flow.md) - Client gameplay audio documentation.
- [gameplay-events-and-effects.md](gameplay-events-and-effects.md) - Client gameplay events and visual effects documentation.
<!-- doc-ledger:files:end -->
## Direct Folders
<!-- doc-ledger:folders:start -->
<!-- doc-ledger:folders:end -->

## Stub Files

<!-- doc-ledger:stubs:start -->
<!-- doc-ledger:stubs:end -->

## Related Docs

- [Client](../!INDEX.md)
- [Gameplay Runtime](../gameplay-runtime/!INDEX.md)
- [Match End Flow](../match-end-flow/!INDEX.md)
- [Pickup Presentation](../world-sync/pickup-presentation.md)
- [Hud And Gameplay UI](../hud-and-gameplay-ui.md)

## Notes

This index stays at the client gameplay-event-presentation boundary and does not expand into runtime, HUD, or world-sync ownership. Event presentation is downstream of `RealtimePresentationState`, and the index only summarizes that seam without adding entries.