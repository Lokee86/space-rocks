...
Under hot-lane stress, multiple `ship_delta`, `asteroid_delta`, or `bullet_delta` packets may arrive for the same lane sequence in one poll window. The routing path treats them as separate transport packets, while `HotDeltaChunkAssembler` retains their update arrays by chunk index and emits one complete logical packet only after every chunk is present. Distinct chunks may arrive in any order. Duplicate indices, inconsistent counts, malformed chunk metadata, completed or lower sequences are rejected. A newer sequence supersedes an incomplete older sequence without applying the older partial state. Sequence gaps remain valid, and ship, asteroid, and bullet assembly and sequence tracking are independent.
