...
It owns Rails route/controller behavior, Rails auth/account persistence, internal service endpoints, and Rails/Postgres physical schema through migrations. It does not own realtime packet schemas, game simulation state, local SQLite storage, or data-sync generated output.
...
| Realtime packet schemas         | `shared/packets/*.toml` owns realtime packet types, packet structs, fields, JSON names, and selected generated outputs.                        |
