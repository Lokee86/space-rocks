...
The 60 Hz session write-loop tick can build and encode active gameplay lane packets. Ship and bullet hot movement emit at 60 Hz for one chunk, 30 Hz for two chunks, and 20 Hz for three or more chunks. Asteroid hot movement emits at 60 Hz when unchunked and 30 Hz when chunking is required. Ship and bullet forced sends may bypass normal cadence suppression; chunked asteroid lifecycle and its related hot/projection transition wait for the permitted asteroid cadence tick. Hot movement is therefore not emitted on every server tick.
