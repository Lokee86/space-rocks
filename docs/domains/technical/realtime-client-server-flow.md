...
gameplay lane packets update gameplay runtime and recipient-relevant world presentation
player_locator updates coarse remote positions used by off-screen indicators
...
The client renders from server-observed facts. Detailed player nodes may leave recipient interest while coarse locator state and cached player/team hues continue driving distinct off-screen indicators. The client does not recalculate authoritative outcomes such as score, collision damage, lives, respawn validity, room match-over, or match results.
