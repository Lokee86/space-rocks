...
The 60 Hz session write-loop tick can build and encode active gameplay lane packets when the session has an active game player, the room has a game instance in an eligible state, and the WebRTC transport is ready. During active candidate construction, movement-only asteroid/bullet output is cadence-gated: `full_owned_30hz` emits every second tick and `full_owned_20hz` every third tick. Non-hot world changes and asteroid/bullet lifecycle changes force immediate related hot emission and projection-chain advancement; hot movement is therefore not emitted on every server tick.
...
For hot `asteroid_delta` and `bullet_delta` packets, the client accepts same-sequence chunks only when each valid `chunk_index` is new for that lane and sequence and `chunk_count` matches the established count. Duplicate indices, inconsistent counts, malformed metadata, and lower sequences are rejected; distinct chunks may arrive in any order, sequence gaps are valid, and the two hot lanes track independently.
