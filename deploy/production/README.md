...
DISCORD_REDIRECT_URI=https://api.laughingskull.ca/api/auth/discord/callback
...
SPACE_ROCKS_WEBRTC_UDP_PORT_MAX=50003
SPACE_ROCKS_WEBRTC_ADVERTISED_UDP_PORT_MIN=replace-with-playit-public-port-min
SPACE_ROCKS_WEBRTC_ADVERTISED_UDP_PORT_MAX=replace-with-playit-public-port-max
...
The four local UDP ports are persistent Pion ICE UDP multiplexers, not one-port-per-player allocations. Peer connections are distributed across the four muxes, and every mux can serve many players. Set `SPACE_ROCKS_WEBRTC_ADVERTISED_IPS` to the Playit public address. Set the advertised UDP range to the four sequential public ports assigned by Playit; the server rewrites each local candidate port to the corresponding public port. An ICE server is not required for this Playit path.
